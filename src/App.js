...
import { 
  ...
  History, Clock, AlertTriangle, Power, Unlock, Menu, Mail 
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribeSettings = onSnapshot(settingsRef, (docSnap) => {
      ...
      else setSystemSettings({ globalLockdown: false, disabledFacilities: [] });
    });
  ...
  const handleLogin = (email) => {
    ...
    alert(`📨 Sending login confirmation code to ${email}... (Simulated)`);
  ...
  const saveBooking = async (booking) => {
    try {
      // SIMULATE BOOKING EMAIL
      alert(`📨 Sending booking confirmation to ${booking.student}... (Simulated)`);
  ...
  const Navbar = () => (
    <nav className={`${user?.role === 'warden' ? 'bg-gray-900' : 'bg-red-600'} text-white p-4 shadow-lg sticky top-0 z-50 flex justify-between items-center`}>
      <div className="flex items-center space-x-2 cursor-pointer" onClick={() => user && setView(user.role === 'warden' ? 'warden' : 'home')}>
        ...
        <span className="font-bold text-xl tracking-tight">WL</span>
      </div>
      
      {/* 3-LINE MENU (HAMBURGER) */}
      {user && (
        <div className="relative" ref={menuRef}>
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-2 rounded-lg hover:bg-black/20 transition-colors">
            <Menu className="h-6 w-6" />
          </button>
          
          {isMenuOpen && (
            <div className={`absolute right-0 mt-2 w-64 rounded-xl shadow-2xl py-2 border animate-fade-in z-50 ${isDarkMode ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-100 text-gray-800'}`}>
              <div className={`px-4 py-3 border-b mb-1 ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}>
                <p className="text-xs uppercase font-bold opacity-50">Signed in as</p>
                <p className="text-sm font-medium truncate">{user.email}</p>
              </div>

              {/* DARK MODE TOGGLE */}
              <button onClick={() => setIsDarkMode(!isDarkMode)} className={`w-full text-left px-4 py-3 text-sm flex items-center transition-colors ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
              ...
              {user.role === 'student' && (
                <>
                  ...
                  <p className="px-4 py-2 text-xs uppercase font-bold opacity-50">Filter Facilities</p>
                  {['All', 'Indoor', 'Outdoor'].map(f => (
                    <button key={f} onClick={() => { setFilter(f); setIsMenuOpen(false); setView('home'); }} className={`w-full text-left px-4 py-2 text-sm flex items-center ${filter === f ? 'text-red-500 font-bold' : ''} ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
                      <Filter size={14} className="mr-3" /> {f}
                    </button>
                  ))}
                  
                  <div className={`my-1 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}></div>
                  <button onClick={() => { setView('mybookings'); setIsMenuOpen(false); }} className={`w-full text-left px-4 py-3 text-sm flex items-center ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
                    <History size={16} className="mr-3" /> My History
                  </button>
                </>
              )}

              {user.role === 'warden' && (
                <button onClick={() => { setView('warden'); setIsMenuOpen(false); }} className={`w-full text-left px-4 py-3 text-sm flex items-center ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
                  <Lock size={16} className="mr-3" /> Admin Panel
                </button>
              )}
              
              <div className={`border-t mt-1 pt-1 ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}>
                <button onClick={handleLogout} className={`w-full text-left px-4 py-3 text-sm text-red-600 flex items-center ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
  ...
  const TicketModal = ({ booking, onClose }) => (
    <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[60] p-4 backdrop-blur-sm" onClick={onClose}>
      <div className={`rounded-3xl p-8 w-full max-w-sm relative animate-fade-in ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`} onClick={e => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600"><X size={24} /></button>
        <div className="text-center">
          <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4"><Activity className="h-8 w-8 text-red-600" /></div>
          <h2 className="text-2xl font-bold mb-1">Entry Ticket</h2>
          <p className="text-sm opacity-60 mb-6">Scan at Gate</p>
          
          {/* BIGGER QR CODE - NO TEXT BELOW */}
          <div className="bg-white p-4 rounded-xl inline-block mb-6 shadow-2xl">
            <img src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${booking.id}`} alt="QR" className="w-64 h-64 rounded-lg" />
          </div>
          
          <div className={`text-left space-y-3 p-5 rounded-2xl border ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-100'}`}>
            <div className="flex justify-between"><span className="opacity-60 text-sm">Facility</span><span className="font-bold text-sm">{booking.facility}</span></div>
            <div className="flex justify-between"><span className="opacity-60 text-sm">Time</span><span className="font-bold text-sm">{booking.time}</span></div>
            <div className="flex justify-between"><span className="opacity-60 text-sm">Status</span><span className={`font-bold text-sm ${booking.price > 0 ? 'text-green-500' : 'text-blue-500'}`}>{booking.status}</span></div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center border-t-8 border-red-600">
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleLogin(e.target.email.value); }}>
          <div className="relative mb-4">
            ...
            <input name="email" type="email" placeholder="id@woxsen.edu.in" className="w-full p-4 pl-12 border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-600 outline-none" required />
          </div>
  ...
  if (view === 'warden') return (
    <div className="min-h-screen font-sans text-gray-900 pb-10 bg-gray-100">
      ...
      <main className="container mx-auto px-4 py-6 max-w-6xl">
        {/* Verification & Dashboard Code same as before but wrapped in light mode only since admin is usually desktop */}
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-8"><h3 className="font-bold text-gray-800 text-lg mb-4 flex items-center"><Power className="mr-2 text-gray-500"/> Maintenance Controls</h3><div className={`flex justify-between items-center p-4 rounded-xl border-2 transition-all mb-6 ${systemSettings.globalLockdown ? 'bg-red-50 border-red-500' : 'bg-green-50 border-green-500'}`}><div><h4 className={`font-bold text-lg ${systemSettings.globalLockdown ? 'text-red-700' : 'text-green-700'}`}>{systemSettings.globalLockdown ? 'SYSTEM LOCKED DOWN' : 'SYSTEM OPERATIONAL'}</h4><p className="text-sm text-gray-600">{systemSettings.globalLockdown ? 'No students can book any slots.' : 'Bookings are open.'}</p></div><button onClick={toggleGlobalLock} className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors ${systemSettings.globalLockdown ? 'bg-red-600' : 'bg-green-600'}`}><span className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${systemSettings.globalLockdown ? 'translate-x-7' : 'translate-x-1'}`} /></button></div><h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Individual Facilities</h4><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">{FACILITIES.map(f => { const isLocked = systemSettings.disabledFacilities?.includes(f.name); return ( <div key={f.id} className="flex justify-between items-center p-3 border rounded-lg bg-gray-50"><span className="font-medium text-sm text-gray-700">{f.name}</span><button onClick={() => toggleFacility(f.name)} className={`p-2 rounded-lg transition-colors ${isLocked ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600 hover:bg-gray-200'}`}>{isLocked ? <Lock size={16} /> : <Unlock size={16} />}</button></div> ) })}</div></div>
        <div className="bg-white rounded-xl shadow overflow-hidden"><div className="p-4 border-b bg-gray-50 flex justify-between items-center"><h3 className="font-bold text-gray-700">Live Booking Feed</h3></div><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-gray-100 text-gray-600 uppercase text-xs"><tr><th className="p-4">Student</th><th className="p-4">Facility</th><th className="p-4">Time</th><th className="p-4">Status</th><th className="p-4">Action</th></tr></thead><tbody className="divide-y divide-gray-100">{myBookings.filter(b => {const isToday = b.date === new Date().toLocaleDateString(); if (!isToday) return false; const currentHour = new Date().getHours(); const bookingHour = parseInt(b.time.split(':')[0]); if (bookingHour < 6 && currentHour > 18) return true; return bookingHour >= currentHour; }).map(b => (<tr key={b.id} className="hover:bg-gray-50"><td className="p-4 font-medium text-gray-900">{b.student.split('@')[0]}</td><td className="p-4">{b.facility}<br/><span className="text-xs text-gray-500">{b.court}</span></td><td className="p-4">{b.time}</td><td className="p-4"><span className={`px-2 py-1 rounded-full text-xs font-bold ${b.price > 0 ? 'bg-green-100 text-green-700' : 'bg-blue-50 text-blue-700'}`}>{b.status}</span></td><td className="p-4"><button onClick={() => deleteBooking(b.id)} className="text-red-400 hover:text-red-600"><Trash2 size={16} /></button></td></tr>))}</tbody></table></div></div>
      </main>
    </div>
  );
  ...
  // --- STUDENT VIEW ---
  return (
    <div className={`min-h-screen font-sans pb-10 transition-colors duration-300 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      <Navbar />
      ...
      <main className="container mx-auto px-4 py-6 max-w-5xl">
        {view === 'home' && (
          <>
            ...
            <div className="flex flex-col md:flex-row justify-between items-end mb-8 pt-4">
              <div><h1 className="text-3xl font-extrabold">Welcome, {user.name}</h1><p className={`text-sm mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Book your slot at SportX.</p></div>
              {/* FILTERS MOVED TO MENU - DELETED FROM HERE */}
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{FACILITIES.filter(f => filter === 'All' || f.type === filter).map(f => { const isLocked = systemSettings.globalLockdown || systemSettings.disabledFacilities?.includes(f.name); return ( <div key={f.id} className={`rounded-2xl shadow-sm border relative overflow-hidden transition-all duration-300 ${isLocked ? 'opacity-75 grayscale' : 'hover:shadow-xl cursor-pointer group transform hover:-translate-y-1'} ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`} onClick={() => { if (!isLocked) { setSelectedFacility(f); setSelectedCourt(f.courts[0]); setView('booking'); } else { alert("Facility Closed"); } }}>{isLocked && ( <div className="absolute inset-0 z-20 flex items-center justify-center bg-gray-900/10 backdrop-blur-[1px]"><div className="bg-red-600 text-white px-3 py-1 rounded-full text-xs font-bold flex items-center shadow-lg"><Lock size={12} className="mr-1"/> CLOSED</div></div> )}<div className="h-48 relative overflow-hidden rounded-t-2xl"><img src={f.image} className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-500" alt={f.name} onError={(e) => e.target.src = f.fallback} /><div className="absolute top-2 right-2 bg-black/60 text-white text-xs px-2 py-1 rounded backdrop-blur-sm flex items-center"><MapPin size={10} className="mr-1"/> {f.type}</div></div><div className="p-5"><h3 className="font-bold text-lg mb-1 truncate">{f.name}</h3><div className={`flex justify-between text-xs mt-3 font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}><span className={`flex items-center px-2 py-1 rounded ${isDarkMode ? 'bg-green-900/30 text-green-400' : 'bg-green-50 text-green-700'}`}><Sun size={12} className="mr-1"/> Free</span><span className={`flex items-center px-2 py-1 rounded ${isDarkMode ? 'bg-indigo-900/30 text-indigo-400' : 'bg-indigo-50 text-indigo-700'}`}><Moon size={12} className="mr-1"/> ₹{f.moonlightPrice}</span></div></div></div> ); })}</div>
          </>
        )}

        {view === 'booking' && (
          <div className={`rounded-2xl shadow-lg p-6 md:p-8 animate-fade-in max-w-2xl mx-auto ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <button onClick={() => { setView('home'); setSelectedSlot(null); }} className={`mb-6 text-sm font-medium flex items-center ${isDarkMode ? 'text-gray-400 hover:text-white' : 'text-gray-400 hover:text-gray-900'}`}>← Back to Facilities</button><h2 className="text-3xl font-bold mb-2">{selectedFacility.name}</h2>
            {(() => {
              ...
              return (
                <>
                  ...
                  <div className="mb-8"><label className="block text-xs font-bold opacity-50 uppercase tracking-wider mb-3">Select Court / Table</label><div className="grid grid-cols-2 md:grid-cols-3 gap-3">{selectedFacility.courts.map(court => (<button key={court} onClick={() => { setSelectedCourt(court); setSelectedSlot(null); }} className={`py-2 px-3 rounded-lg text-sm font-medium border transition-all ${selectedCourt === court ? 'bg-gray-200 text-black border-gray-200' : isDarkMode ? 'bg-gray-700 text-gray-300 border-gray-600 hover:bg-gray-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}>{court}</button>))}</div></div>
                  <div className="mb-8"><label className="block text-xs font-bold opacity-50 uppercase tracking-wider mb-3">Select Time (Today)</label><div className="grid grid-cols-3 md:grid-cols-4 gap-3">{selectedFacility.slots.map((slot) => { const hour = parseInt(slot.split(':')[0]); const isMoonlight = hour >= 0 && hour < 6; const isTaken = myBookings.some(b => b.facility === selectedFacility.name && b.court === selectedCourt && b.time === slot && b.date === new Date().toLocaleDateString()); const isDisabled = isTaken || hasBookingToday; return ( <button key={slot} disabled={isDisabled} onClick={() => handleSlotSelect(slot)} className={`py-3 px-2 rounded-xl text-sm font-medium transition-all relative flex flex-col items-center justify-center border-2 ${isDisabled ? 'bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed opacity-60' : selectedSlot === slot ? 'border-red-600 bg-red-50 text-red-600' : isDarkMode ? 'border-transparent bg-gray-700 text-gray-300 hover:bg-gray-600' : 'border-transparent bg-gray-50 text-gray-600 hover:bg-gray-100'}`}><div className="flex items-center space-x-1 mb-1">{isMoonlight ? <Moon size={12} /> : <Sun size={12} />}<span className={isTaken ? 'line-through' : ''}>{slot}</span></div><span className={`text-[10px] uppercase font-bold ${isDisabled ? 'text-gray-400' : isMoonlight ? 'text-indigo-500' : 'text-green-500'}`}>{isTaken ? 'BOOKED' : hasBookingToday ? 'LIMIT' : isMoonlight ? `₹${selectedFacility.moonlightPrice}` : 'FREE'}</span></button> ); })}</div></div>
                  <div className={`border-t pt-6 ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}><div className="flex justify-between items-center mb-6"><div><p className="text-xs opacity-50 uppercase font-bold">Total Payable</p><p className="text-3xl font-black">₹{currentPrice}</p></div>{currentPrice > 0 && <span className="bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full text-xs font-bold">Moonlight Fee</span>}</div><button disabled={!selectedSlot} onClick={handlePayment} className="w-full py-4 rounded-xl font-bold text-lg bg-red-600 text-white shadow-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed">{currentPrice > 0 ? `Pay ₹${currentPrice}` : 'Confirm Booking'}</button></div>
                </>
              );
            })()}
          </div>
        ...
        {view === 'mybookings' && (
          <div className="max-w-2xl mx-auto">
            <div className="flex items-center mb-6"><button onClick={() => setView('home')} className={`mr-4 ${isDarkMode ? 'text-gray-400 hover:text-white' : 'text-gray-400 hover:text-gray-900'}`}>←</button><h2 className="text-2xl font-bold">My History</h2></div>
            <div className="space-y-4 mb-8">{myBookings.filter(b => b.student === user.email).map(b => (<div key={b.id} className={`p-5 rounded-xl shadow-sm border flex justify-between items-center ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-green-100'}`}><div className="flex items-center space-x-3"><div className="h-10 w-1 bg-green-500 rounded-full"></div><div><h3 className="font-bold">{b.facility}</h3><p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{b.court} • {b.time}</p></div></div><button onClick={() => setTicketModal(b)} className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center ${isDarkMode ? 'bg-gray-700 text-white hover:bg-gray-600' : 'text-white bg-black'}`}><QrCode size={14} className="mr-2"/> Ticket</button></div>))}</div>
          </div>
        )}
      </main>
    </div>
